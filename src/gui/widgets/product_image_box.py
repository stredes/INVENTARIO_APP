...
class ProductImageBox(ttk.Frame):
    ...
    - set_product(product_id: Optional[int], on_image_changed: Optional[Callable[[Optional[Path]], None]])
    - Muestra miniatura y permite cargar/ver/quitar.
    """
    ...
    def __init__(self, master: tk.Misc, width: int = 220, height: int = 220, **kw):
        ...
        # ¡IMPORTANTE! No usar nombres reservados por Tkinter (_w) para atributos propios
        self._box_w = width
        self._box_h = height
        self._imgtk: Optional[ImageTk.PhotoImage] = None

        # Canvas de preview
        self.canvas = tk.Canvas(self, width=self._box_w, height=self._box_h, bg="#f4f4f4", highlightthickness=0)
    ...
    def _open(self):
        ...
        if main:
            import webbrowser
            try:
                webbrowser.open(main.as_uri())
            except Exception:
                webbrowser.open(str(main))

    def _clear(self):
        # Solo limpia el preview; si quieres borrar de disco, hazlo desde el callback.
        self._draw_empty()
        if self._on_changed:
    ...
    def _draw_empty(self):
        ...
        self.canvas.create_text(self._box_w // 2, self._box_h // 2, text="Sin imagen", fill="#888")

    def _draw(self, path: Path):
        try:
            im = Image.open(path).convert("RGB")
            im.thumbnail((self._box_w, self._box_h))
            self._imgtk = ImageTk.PhotoImage(im)
            self.canvas.delete("all")
            self.canvas.create_image(self._box_w // 2, self._box_h // 2, image=self._imgtk)
        except Exception:
            self._draw_empty()
