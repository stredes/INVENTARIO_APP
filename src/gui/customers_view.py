...
class CustomersView(ttk.Frame):
    ...
    def __init__(self, master: tk.Misc):
        ...
        contact = ttk.LabelFrame(frm, text="Contacto", padding=10)
        contact.grid(row=0, column=1, sticky="nsew", padx=(8, 0), pady=(0, 8))
        address = ttk.LabelFrame(frm, text="Dirección", padding=10)
        ...
        self.ent_razon = row(identity, "Razón social", self.var_razon, 0)
        row(identity, "RUT", self.var_rut, 1, width=20)
        row(contact, "Contacto", self.var_contacto, 0)
        row(contact, "Teléfono", self.var_telefono, 1)
        row(contact, "Email", self.var_email, 2)
        row(address, "Dirección", self.var_direccion, 0, width=90)
