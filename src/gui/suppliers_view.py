...
class SuppliersView(ttk.Frame):
    ...
    def _on_add(self):
        try:
            ...
            if not data: return
            s = Supplier(**data)
            self.session.add(s); self.session.commit()
            self._clear_form(); self._load_table()
            messagebox.showinfo("OK", f"Proveedor '{data['nombre']}' creado.")
    ...
    def _on_delete(self):
        if self._editing_id is None: return
        if not messagebox.askyesno("Confirmar", "¿Eliminar este proveedor?"): return
        try:
    ...
    def _on_row_dblclick(self, _=None):
        ...
        self.var_contacto.set(vals[2] or "")
        self.var_telefono.set(vals[3] or "")
        self.var_email.set(vals[4] or "")
        self.var_direccion.set(vals[5] or "")
