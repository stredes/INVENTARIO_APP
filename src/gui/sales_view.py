...
class SalesView(ttk.Frame):
    ...
    def __init__(self, master: tk.Misc):
        ...
        self.cmb_customer = ttk.Combobox(top, state="readonly", width=40)
        self.cmb_customer.grid(row=0, column=1, sticky="w", padx=4, pady=4)

        self.var_apply = tk.BooleanVar(value=True)
        ttk.Checkbutton(top, text="Descontar stock (Confirmada)", variable=self.var_apply).grid(row=0, column=2, padx=10)
        ...
        self.tree = ttk.Treeview(self, columns=("prod_id","producto","cant","precio","subtotal"), show="headings", height=10)
        for cid, text, w, anchor in [("prod_id","ID",60,"center"),("producto","Producto",300,"w"),("cant","Cant.",80,"e"),("precio","Precio",100,"e"),("subtotal","Subtotal",120,"e")]:
            self.tree.heading(cid, text=text); self.tree.column(cid, width=w, anchor=anchor)
        ...
        ttk.Button(bottom, text="Generar OV (PDF en Descargas)", command=self._on_generate_so_downloads).pack(side="right", padx=6)
        ttk.Button(bottom, text="Confirmar venta", command=self._on_confirm_sale).pack(side="right", padx=6)
        ...
        self.ent_sale_id = ttk.Entry(admin, width=10); self.ent_sale_id.grid(row=0, column=1, sticky="w", padx=4, pady=4)
        ttk.Button(admin, text="Cancelar (reversa stock si confirmada)", command=self._on_cancel_sale).grid(row=0, column=2, padx=6)
        ttk.Button(admin, text="Eliminar (reversa si confirmada)", command=self._on_delete_sale).grid(row=0, column=3, padx=6)

        self.refresh_lookups()

    def refresh_lookups(self):
        self.customers = self.session.query(Customer).order_by(Customer.nombre.asc()).all()
        self.cmb_customer["values"] = [f"{c.id} - {c.nombre}" for c in self.customers]
    ...
    def _get_selected_customer_dict(self) -> dict:
        idx = self.cmb_customer.current()
        if idx < 0: raise RuntimeError("Seleccione un cliente")
        c = self.customers[idx]
        return {"id": c.id, "nombre": c.nombre, "contacto": c.contacto, "telefono": c.telefono, "email": c.email, "direccion": c.direccion}

    def _on_confirm_sale(self):
        try:
            items = self._collect_items()
            if not items: messagebox.showwarning("Validación", "Agregue al menos un ítem."); return
            cust = self._get_selected_customer_dict()
            sm_items = [SaleItem(product_id=it["id"], cantidad=it["cantidad"], precio_unitario=it["precio"]) for it in items]
            self.sm.create_sale(
                customer_id=cust["id"],
                items=sm_items,
    ...
    def _on_generate_so_downloads(self):
        try:
            ...
            out = generate_so_to_downloads(
                so_number=so_number, customer=cust, items=items, currency="CLP", notes=None, auto_open=True
            )
    ...
    def _on_delete_sale(self):
        try:
            sid = int(self.ent_sale_id.get())
            if not messagebox.askyesno("Confirmar", f"¿Eliminar venta {sid}?"): return
