...
from src.utils.po_generator import generate_po_to_downloads as _gen_po


def guardar_documento(conn: sqlite3.Connection, header: Dict[str, Any], items: List[Dict[str, Any]], *, usuario: str = "system") -> int:
    return _save_base(conn, "OC", header, items, usuario=usuario)
...
def generar_pdf(conn: sqlite3.Connection, id_documento: int, *, auto_open: bool = True) -> str:
    ...
    return _gen_po(
        po_number=str(doc["folio"] or f"OC-{doc['id']}"),
        supplier=supplier,
        items=items,
