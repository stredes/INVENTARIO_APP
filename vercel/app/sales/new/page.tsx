...
type Product = { id: number; nombre: string; sku: string; precio_venta: number };
...
export default function NewSalePage() {
  ...
  const addItem = async () => {
    ...
    try {
      ...
      const price = Number((p as any).precio_venta || 0);
      setItems([...items, { product: p, product_id: p.id, cantidad: qty, precio_unitario: price }]);
  ...
  const onSubmit = async () => {
    ...
    try {
      if (!customerId) throw new Error('Debe indicar cliente');
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 12 }}>
        <label>Cliente ID <input value={customerId} onChange={(e) => setCustomerId(e.target.value)} placeholder="ID Cliente" /></label>
      </div>
      ...
      <div style={{ marginTop: 16, display: 'flex', gap: 8 }}>
        <button disabled={loading} onClick={onSubmit}>Confirmar y Descontar Stock</button>
