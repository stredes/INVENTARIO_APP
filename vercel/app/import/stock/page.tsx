...
export default function ImportStockPage() {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      const r = await fetch(`${apiBase}/import/stock?mode=${mode}`, { method: 'POST', body: fd });
  ...
  return (
    <div>
      <h1>Importar Stock (CSV)</h1>
      <p className="muted">Cabecera esperada: sku,id,stock — usa SKU o ID. Modo <b>set</b> fija el stock, <b>add</b> suma.</p>
      {err && <p style={{ color: 'crimson' }}>Error: {err}</p>}
      <div className="card pad" style={{ display:'grid', gap:8, maxWidth: 720 }}>
        <div style={{ display:'flex', gap:8 }}>
          <label>Modo
            <select value={mode} onChange={(e)=> setMode(e.target.value as any)}>
              <option value="set">Fijar (set)</option>
              <option value="add">Sumar (add)</option>
            </select>
          </label>
      ...
      {res && (
        <div className="card pad" style={{ marginTop: 12 }}>
          <div>Cambios: <b>{res.changed}</b></div>
          {res.errors?.length ? (
